...
class ConvBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
                discriminator = False,
                use_act = True,    # In the residual block second part does not have activation layer
                use_bn = True,
                **kwargs,):    
...
class Discriminator(nn.Module):
    def __init__(self, in_channels = 3, features = [64, 64, 128, 128, 256, 256, 512, 512]):
        ...
        for idx, feature in enumerate(features):
            blocks.append(ConvBlock(in_channels, feature, kernel_size = 3, stride = 1 + idx % 2, padding = 1,
                                    discriminator = True, use_act = True, use_bn = False if idx == 0 else True,))  # First block do not have BN
            in_channels = feature
...
def test():
    ...
    with torch.cuda.amp.autocast(enabled=(device=="cuda")):
        ...
        x = torch.randn((5, 3, low_resolution, low_resolution)).to(device)
